...
async function scanGit(url, scanId) {
    try {
        ...
        log('[trufflehog] Git scan error for', url, ':', err.message);
        return 0;
    }
}
async function scanWebsite(domain, scanId) {
    try {
        ...
        log('[trufflehog] Website scan error for', domain, ':', err.message);
        return 0;
    }
}
async function scanFiles(domain, scanId) {
...
export async function runTrufflehog(job) {
    ...
    try {
        ...
        log('[trufflehog] Error during secret scan:', err.message);
        await insertArtifact({
            type: 'secret',
