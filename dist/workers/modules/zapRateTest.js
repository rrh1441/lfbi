      ...
      `, [job.scanId]);
        ...
        for (const endpoint of testableEndpoints) {
            try {
                ...
                let testData = { test: 'rate_limit_check' };
                ...
                for (let i = 0; i < requestCount; i++) {
                    ...
                    rapidRequests.push(requestPromise.catch(error => ({ error: true, message: error.message })));
                }
                ...
                if (validResponses > 0 && notFoundResponses < validResponses * 0.8) {
                    ...
                    if (successCount > requestCount * 0.75) {
                        ...
                        await insertFinding(artifactId, 'RATE_LIMITING', 'Implement proper rate limiting to prevent abuse. Use techniques like token bucket, sliding window, or per-IP limits.', `Endpoint ${endpoint.path} (${endpoint.method}) allows ${successCount}/${requestCount} rapid requests without proper rate limiting`);
                        findingsCount++;
                ...
                log(`[zapRateTest] Error testing ${endpoint.path}:`, error.message);
            }
        }
