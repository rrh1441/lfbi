...
export async function runZapRateTest(job) {
    ...
    try {
        ...
        for (const baseUrl of baseUrls) {
            for (const endpoint of testEndpoints) {
                try {
                    ...
                    const rapidRequests = [];
                    for (let i = 0; i < 20; i++) {
                        rapidRequests.push(axios.post(url, { test: 'rate_limit_check', attempt: i }, {
                            timeout: 5000,
                    ...
                    if (successCount > 15) {
                        ...
                        await insertFinding(artifactId, 'RATE_LIMITING', 'Implement proper rate limiting to prevent abuse. Use techniques like token bucket, sliding window, or per-IP limits.', `Endpoint ${endpoint} allows ${successCount}/20 rapid requests without proper rate limiting`);
                        findingsCount++;
        ...
        try {
            ...
            for (const endpoint of formEndpoints) {
                ...
                try {
                    ...
                    if (formTest.status === 200 && !formTest.data.includes('captcha')) {
                        ...
                        await insertFinding(artifactId, 'FORM_SECURITY', 'Implement CAPTCHA or similar bot protection on forms to prevent automated abuse.', `Form at ${endpoint} may lack CAPTCHA protection against automated submissions`);
                        findingsCount++;
                    }
                }
                catch (formError) {
            ...
            log('[zapRateTest] CAPTCHA testing failed:', error.message);
        }
        log('[zapRateTest] Rate limiting tests completed, found', findingsCount, 'issues');
        return findingsCount;
    }
    catch (error) {
        log('[zapRateTest] Error during rate limit testing:', error.message);
        return 0;
    }
}
//# sourceMappingURL=zapRateTest.js.map