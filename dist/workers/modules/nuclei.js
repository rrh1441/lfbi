    ...
    try {
        ...
        for (const line of findings) {
            try {
                ...
                await insertFinding(artifactId, 'VULNERABILITY', getRecommendation(vuln.info.name, vuln['template-id']), `${vuln.info.description || vuln.info.name} detected on ${job.domain}`);
                findingsCount++;
            }
            catch (parseError) {
        ...
        log('[nuclei] Error during scan:', error.message);
        return 0;
    }
}
function getRecommendation(vulnName, templateId) {
    ...
    const key = Object.keys(recommendations).find(k => vulnName.toLowerCase().includes(k) || templateId.includes(k));
