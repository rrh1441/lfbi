...
export async function runShodanScan(domain, scanId, companyName) {
    ...
    try {
        ...
        for (const target of allTargets) {
            ...
            try {
                ...
                console.log(`[shodan] Search failed for ${target}:`, error.message);
            }
        }
        ...
        console.error('[shodan] Scan failed:', error.message);
        // Create error artifact
        await insertArtifact({
            type: 'scan_error',
            val_text: `Shodan scan failed: ${error.message}`,
            severity: 'INFO',
            meta: {
                scan_id: scanId,
                company: companyName,
...
async function getDiscoveredTargets(scanId) {
    try {
    ...
    `;
        ...
        console.log('[shodan] Failed to get discovered targets:', error.message);
        return [];
    }
}
async function processShodanResults(matches, scanId, companyName, searchTarget) {
    let findings = 0;
    for (const result of matches) {
        ...
        for (const finding of findingsList) {
            await insertFinding(artifactId, 'vulnerability', getShodanRecommendation(result.port, finding), finding);
        }
        findings++;
    }
    return findings;
}
async function searchByOrganization(companyName, scanId, apiKey) {
    const orgSearchUrl = `https://api.shodan.io/shodan/host/search?key=${apiKey}&query=org:"${companyName}"`;
    try {
        const orgResponse = await axios.get(orgSearchUrl, {
            timeout: 30000,
            headers: {
                'User-Agent': 'DealBrief-Scanner/1.0'
            }
        ...
        console.log('[shodan] Organization search failed:', orgError.message);
        return 0;
    }
}
function getShodanRecommendation(port, finding) {
