...
async function processScan(job) {
    ...
    try {
        ...
        try {
            ...
            log(`SpiderFoot discovery failed:`, error.message);
        ...
        try {
            ...
            log(`DNS Twist failed:`, error.message);
        }
        ...
        try {
            ...
            log(`CRM/File hunting failed:`, error.message);
        }
        ...
        try {
            ...
            log(`Shodan scan failed:`, error.message);
            throw new Error(`Real Shodan scan failed: ${error.message}`);
        }
        ...
        try {
            ...
            log(`Database scan failed:`, error.message);
        }
        ...
        try {
            ...
            log(`Endpoint discovery failed:`, error.message);
        }
        ...
        try {
            ...
            log(`TLS scan failed:`, error.message);
        }
        ...
        try {
            ...
            log(`Nuclei scan failed:`, error.message);
        }
        ...
        try {
            ...
            log(`Rate testing failed:`, error.message);
        }
        ...
        try {
            ...
            log(`Email security scan failed:`, error.message);
        }
        ...
        try {
            ...
            log(`Secret detection failed:`, error.message);
        }
        ...
        try {
            ...
            log(`⚠️ Report generation failed but scan data preserved:`, reportError.message);
            // Continue - scan data is still valid even if report generation fails
        }
        // Mark scan as complete ONLY with real data
        await queue.updateStatus(scanId, 'done', `Comprehensive security scan completed - ${totalFindings} verified findings across 8 security modules. AI reports generated and ready for editing.`, '' // Remove the old PDF path since we now have formatted text reports
        );
        log(`✅ COMPREHENSIVE SCAN COMPLETED for ${companyName}: ${totalFindings} verified findings across all security modules`);
    }
    catch (error) {
        log(`❌ Scan failed for ${companyName}:`, error.message);
        await queue.updateStatus(scanId, 'failed', `Scan failed: ${error.message}`);
        // Store ONLY the error - no fake data
        await insertArtifact({
            type: 'scan_error',
            val_text: `Comprehensive scan failed: ${error.message}`,
            severity: 'INFO',
            meta: {
                scan_id: scanId,
                company: companyName,
...
startWorker().catch(error => {
    log('CRITICAL: Failed to start worker:', error.message);
    process.exit(1);
});
...
async function generateDualModelReports(scanId, companyName, domain) {
    try {
...
Follow the DealBrief format exactly. Focus on material business risks, not theoretical concerns. Use plain English and cite sources properly.`;
        ...
        log(`❌ Dual-model report generation failed:`, error.message);
        throw error;
    }
}
