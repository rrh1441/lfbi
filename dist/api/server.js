...
fastify.post('/scan', async (request, reply) => {
    const { companyName, domain } = request.body;
...
fastify.get('/scan/:scanId/report', async (request, reply) => {
    ...
    try {
        ...
        log('[api] Report generation error:', error.message);
        reply.status(500);
...
fastify.get('/scan/:scanId/summary', async (request, reply) => {
    ...
    try {
        ...
        log('[api] Summary generation error:', error.message);
        reply.status(500);
        return { error: 'Failed to generate summary', details: error.message };
    }
});
...
fastify.post('/scan/:id/callback', async (request, reply) => {
    try {
        ...
        log('[api] Error handling callback:', error.message);
