...
export async function generateSecurityReport(scanId, companyName, domain) {
    try {
...
export async function generateExecutiveSummary(scanId, companyName) {
    try {
        // STRICT VALIDATION
        const validation = await validateScanData(scanId);
        if (!validation.isValid) {
        ...
        const artifactsResult = await pool.query('SELECT severity, COUNT(*) as count FROM artifacts WHERE meta->>\'scan_id\' = $1 AND meta->>\'error\' IS NULL GROUP BY severity', [scanId]);
