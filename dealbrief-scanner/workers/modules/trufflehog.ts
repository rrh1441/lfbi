...
async function scanGit(url: string) {
...
export async function runTrufflehog(job: { domain: string }) {
  ...
  try {
    const links = JSON.parse(
      await fs.readFile('/tmp/spiderfoot-links.json', 'utf8')
    ) as string[];
    for (const l of links) if (GITHUB_RE.test(l)) await scanGit(l);
  } catch {}
}
