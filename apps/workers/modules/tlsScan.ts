  ...
  try {
    ...
    if (/TLSv1[01]/.test(stdout) || /RC4|3DES/.test(stdout)) {
      ...
      await insertFinding(
        artifactId,
        'TLS_SECURITY',
        'Upgrade to TLS 1.2+ and disable weak ciphers (RC4, 3DES). Update server configuration to only allow secure cipher suites.',
        'Legacy TLS protocols or weak ciphers detected'
      );
    ...
    if (certMatch) {
      ...
      if (year <= currentYear) {
        ...
        await insertFinding(
          artifactId,
          'CERTIFICATE_EXPIRY',
          'Renew SSL certificate before expiration to avoid service interruption. Set up automatic renewal if possible.',
          `SSL certificate expires in ${year}`
        );
    ...
    if (/VULNERABLE/.test(stdout)) {
      ...
      await insertFinding(
        artifactId,
        'TLS_VULNERABILITY',
        'Review and patch TLS implementation. Update server software and configuration.',
        'TLS vulnerability detected by testssl.sh'
      );
