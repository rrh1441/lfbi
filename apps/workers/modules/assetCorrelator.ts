...
async function correlateAssets(scanId: string, domain: string): Promise<void> {
  ...
  // Optimized query with extracted JSON fields
  ...
  // Phase 1: Collect all hostnames for batch DNS resolution
  const allHostnames = new Set<string>();
  const artifactBuffer: RawArtifact[] = [];

  await new Promise<void>((resolve, reject) => {
    stream.on('data', (row: RawArtifact) => {
      artifactCount++;
