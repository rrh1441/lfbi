  ...
  try {
    ...
    if (!dmarcOut.trim()) {
      ...
      await insertFinding(
        artifactId,
        'EMAIL_SECURITY',
        'Implement DMARC policy to prevent email spoofing. Start with p=none for monitoring, then progress to p=quarantine or p=reject.',
      ...
      await insertFinding(
        artifactId,
        'EMAIL_SECURITY',
        'Strengthen DMARC policy from p=none to p=quarantine or p=reject after monitoring email flow.',
        'DMARC policy set to none - provides limited protection against email spoofing'
      );
    ...
    if (!spfRecord) {
      ...
      await insertFinding(
        artifactId,
        'EMAIL_SECURITY',
        'Implement SPF record to specify authorized mail servers. Use tools like SPF Record Generator to create proper policy.',
        'SPF record not found - domain vulnerable to email spoofing'
      );
      ...
      if (spfRecord.includes('~all') || spfRecord.includes('?all')) {
        ...
        await insertFinding(
          artifactId,
          'EMAIL_SECURITY',
          'Strengthen SPF policy by using "-all" instead of "~all" or "?all" for better email authentication.',
          'SPF policy uses soft fail (~all) or neutral (?all) - allows spoofed emails'
        );
      ...
      if (totalLookups > 8) {
        ...
        await insertFinding(
          artifactId,
          'EMAIL_SECURITY',
          'Simplify SPF record to stay under 10 DNS lookups limit. Use IP addresses instead of includes where possible.',
          `SPF record has ${totalLookups} DNS lookups - may exceed RFC limit of 10`
        );
    ...
    if (!dkimFound) {
      ...
      await insertFinding(
        artifactId,
        'EMAIL_SECURITY',
        'Implement DKIM signing for email authentication. Configure DKIM in your email service or mail server.',
        'DKIM signatures not detected for common selectors'
      );
    ...
    log('[spfDmarc] Error during scan:', (error as Error).message);
    return 0;
  }
} 