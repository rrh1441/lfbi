...
export async function runDbPortScan(job: { domain: string; scanId?: string }): Promise<number> {
    ...
    try {
      ...
      try {
        ...
        await insertFinding(
          artifactId,
          'DATABASE_EXPOSURE',
          `Secure ${type} by restricting network access, implementing VPN, or using authentication. Database services should not be exposed to the internet.`,
          `${type} database service exposed on port ${port}`
        );
          ...
          try {
            ...
            if (/Accounts:.+Valid:\s+[1-9]/i.test(stdout)) {
              ...
              await insertFinding(
                credArtifactId,
                'WEAK_CREDENTIALS',
                'Change default database credentials immediately. Use strong, unique passwords and enable proper authentication.',
                `Default or weak ${type} credentials detected`
              );
              
              findingsCount++;
            }
          } catch (bruteError) {
            log(`[dbPortScan] Brute force failed for ${host}:${port}:`, (bruteError as Error).message);
          }
        }
